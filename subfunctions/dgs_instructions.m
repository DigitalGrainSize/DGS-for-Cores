''Digital Grain Size Graphical User Interface for Cores''
...
Please ensure you have the following files in the same directory as the main program, 'dgs_core_gui.m':
...
2. Type 'dgs_core_gui' (without the apostrophes) in the command window to initiate the program
...
Choose the whole image by pressing the 'ROI whole' button.
...
8. When the results are in, the grain size distribution will be shown in the right window, for all the rows processed. 
The mean grain size will be displayed as a red line.
